...
- **视觉主题**：内置「夜祷 / 晨曦」两套，支持复制、编辑、设为默认；样式按 Theme → 类型默认 → 段落覆盖三级就近级联。
- **段落样式面板**：背景色/背景图、标题/正文/启应字体字号颜色对齐、边距；可「设为本类型默认」写入当前主题。
...
- 阶段二（已实现内容库与模板部分）：赞美诗库 / 礼文库 / 视觉主题 / 流程模板管理（含导入导出）/ 样式面板 / 媒体子系统 / 近似预览。
