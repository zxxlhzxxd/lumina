...
### 1.3 核心价值
- 内置整本圣经（简体中文 1919 和合本神版，公有领域），输入经文范围即可自动生成对应 PPT 页。
- 内置常见赞美诗歌词库，一键生成赞美诗 PPT 段落。
- 礼拜流程抽象为可复用的「模板」，每周套用模板后只需填充变化内容。
- 统一默认样式（背景、字体、排版），保证每周 PPT 风格一致。
...
这是整个工具的核心。礼拜 PPT 被抽象为「工程（Project）→ 段落（Section）→ 幻灯片（Slide）」三层结构，叠加「模板（Template）」复用机制和内置默认视觉样式。
...
flowchart LR
    ...
    DefaultStyle[内置默认样式] -.默认样式.-> SectionStyle
    ServiceTemplate[ServiceTemplate 流程模板] -.实例化.-> Project
```

### 4.1 Project（礼拜工程）
一次礼拜的完整工程，是用户的主要工作单元。
- 字段：`id`、`name`（如「2026-06-14 主日崇拜」）、`date`、`sections[]`（有序段落列表）、`slideSize`（16:9 / 4:3）、`createdAt`、`updatedAt`、`meta`（牧者、主题经文等备注）。
...
- `style`（段落级样式覆盖，见 §4.6）：背景图/视频/音频、字体、字号、颜色、对齐、边距等；未设置项回退到内置默认样式。
- `notes`（同工备注，不进入 PPT）。
...
### 4.6 样式与排版模型（Style）
样式分三级，**就近覆盖**：内置默认样式 → SectionTypeDefault（按类型的默认） → Section（段落级覆盖）。
- 可设置项：背景（纯色 / 图片 / 视频）、背景音频、主字体与字号、标题字体、文字颜色、对齐方式、行距、边距/安全区、阴影/描边（提升投影可读性）、启/应标识样式。
- `slideSize` 在 Project 级（16:9 默认 / 4:3）。

### 4.7 默认视觉样式
应用内置一套全局视觉风格定义：默认背景、配色、字体族、各 Section 类型的默认样式、幻灯片母版风格。用户不需要选择主题；新建工程自动使用该默认样式。
...
> 模板只承担流程/结构复用；视觉样式由内置默认样式与段落级覆盖共同决定。
...
- **Project**：复制为新工程（§5.1）。
- **Section**：工程内复制段落（§5.2）。
- **ServiceTemplate**：复制模板为可编辑副本（便于在内置只读项基础上改造，§5.6）。
- **Hymn / LiturgyText**：复制库条目为新条目（便于基于现有诗歌/礼文改词，§5.4 / §5.5）。

### 5.3 圣经经文功能
- **内置整本圣经**：简体中文 1919 和合本神版（CUV Shen），离线 SQLite；build 可用本地 `.lumina-bible` 替换（例如有权使用的 1988 新标点，不得入库）。
...
- 实现策略（分期）：
  - v1：前端用 HTML/CSS 按默认样式与段落样式渲染「近似预览」（所见接近所得）。
...
### 6.2 母版与版式
- 建立与默认视觉样式对应的 Slide Master / Layout，统一背景、字体、占位框，确保风格一致与可维护。
...
- 版本：简体中文和合本神版（CUV Shen，1919，公有领域文本），以 `.lumina-bible` 入库、build 时导入 sqlite。
- 1988 新标点和合本有版权，不得放入 MIT 仓库；仅允许本地 `--bible` 导入。
- 数据来源需为可合法分发的公有领域语料，并校验完整性（66 卷、章节数正确）与 1919 神版用字（他/大马色/形像等）。
...
### 7.4 模板文件
- 流程模板以 JSON 持久化于应用数据目录，附带内置只读集合与用户可写集合。
...
### 8.2 圣经
- `GET /api/v1/bible/info` — 当前导入译本元数据（id、名称、年份、许可）。
...
- **新建向导**：选择流程模板 → 设置日期/名称 → 进入编辑。
- **段落编辑器**：按类型呈现差异化表单（经文引用输入框、赞美诗检索、礼文选择、媒体选择、样式面板）。
- **样式面板**：背景/字体/字号/颜色/对齐/边距等，应用到当前段落。
...
- **风险 2 — 圣经语料合法性与质量**：默认仅捆绑 1919 神版公有领域文本；1988 新标点不得入库。用字探针防止再误用新标点电子整理本。
...
- **阶段二（内容库与模板）**：赞美诗库 + 礼文库 + 默认视觉样式 + 流程模板管理 + 样式面板 + 近似预览 + **音频按页绑定与按单击顺序播放（一次/循环，强需求，§6.4.2）**。
